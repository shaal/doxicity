...
interface IconAddonOptions {
  /** The class name to add to the icon. Defaults to "icon-addon". */
  className: string;
  /** The URL the icon will link to. */
  url: string;
  /** A title for the icon. */
  title: string;
  /** The link's target. Use _blank to open in a new window. */
  target: '_self' | '_blank';
  /** The icon to use. You can choose from one of the built-in options or provide your own SVG string. */
...
/** Adds an icon addon to the site. */
export default function (options: Partial<IconAddonOptions>): DoxicityPlugin {
  const opts: IconAddonOptions = {
  ...
  return {
    transform: (doc: Document) => {
      const addons = doc.getElementById('docs-icon-addons')!;
      const themeToggle = addons.querySelector('#docs-theme-toggle')!;
      const a = doc.createElement('a');
      const key = opts.icon as keyof typeof icons;
      const icon = icons[key] ? icons[key] : opts.icon;

      a.classList.add(opts.className);
      a.href = opts.url;
      ...
      themeToggle.insertAdjacentElement('beforebegin', a);
